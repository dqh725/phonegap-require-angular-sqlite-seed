define(['angularjs'],function(angularjs){
    ...
    self.lastId;
		self.init = function(){
      // first line is for web sql, while the second is the mobile plugin
        ...
        self.db.transaction(function(transaction) {
            transaction.executeSql(query, bindings, function(transaction, result) {
            		console.log("Returned rows = " + result.rows.length);
						    ...
						    for (var i=0; i<len; i++){
						        console.log("Row = " + i + " ID = " + result.rows.item(i).id + " Data =  " + result.rows.item(i).toString());
						    }
    ...
    self.insert = function(item){
      ...
      var deferred = $q.defer();
  	  self.db.transaction(function (tx) {  
          ...
          // TODO: replace with your own insert sql sentance
          function(tx, result){
            deferred.resolve(result)
          },
          function(e){
